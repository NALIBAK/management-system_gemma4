...
s = requests.Session()
r = s.post("http://localhost:6000/api/auth/login", json={"username": "superadmin", "password": "Admin@123"})
if not r.ok or not r.json().get("success"):
    print("Login failed", r.text)
    exit(1)

# 2. Trigger smart fallback to export the defaulter report
r2 = s.post("http://localhost:6000/api/aira/chat", json={"message": "download fee defaulter report"})
...
if match:
    ...
    if not url.startswith("http"):
        url = "http://localhost:6000" + url.split("?")[0]
